...
json_data = json.loads(open('5_24_5.json', 'r').read())
...
for region in regions:
    ...
    if combine_channels:
        mask_arr = get_mask_from_signal_activation_data(consolidated_signal_activation_data, exploratory, hide_zeroes, data_by_density, 1)
        ax.imshow(mask_arr, cmap='Reds', aspect="auto", interpolation="nearest")
        mask_arr_2 = get_mask_from_signal_activation_data(consolidated_signal_activation_data, exploratory, hide_zeroes, data_by_density, 2)
        ax.imshow(mask_arr_2, cmap='Greens', aspect="auto", interpolation="nearest")
